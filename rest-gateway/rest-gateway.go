...
func main() {
	...
	mux.HandleFunc("/swagger/", swaggerServer("pb"))
	mux.HandleFunc("/healthz", healthzServer(cc))

	gwMux := gwruntime.NewServeMux()
	errGw := calcpb.RegisterCalcHandler(context.Background(), gwMux, cc)
	if errGw != nil {
		log.Panic(err)
	}
	mux.Handle("/", gwMux)
...
func swaggerServer(dir string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		if strings.HasSuffix(r.URL.Path, "swagger/") {
